<template>
  <div>
    <h2>{{ title }}</h2>
    <p>
      The <InlineCode>{{ requestName }}</InlineCode> lets you define scale/resolution/zoom level
      limits for features on that layer.
    </p>
    <p>In this example the map is initialized with the following code adding two vector layers:</p>
    <ul>
...
channel.postRequest('MapModulePlugin.AddFeaturesToMapRequest',
  [geojsonPolygons, {
    ...
    There should a polygon and a point initialized on the map when this example is loaded. They are
    both visible at zoom level 7. If you zoom in the polygon isn't shown. If you zoom out the point
    isn't shown. If you get lost click the button below:<br />
    ...
    <p>
      You can also use <InlineCode>minScale</InlineCode>, <InlineCode>maxScale</InlineCode>,
      <InlineCode>minResolution</InlineCode>, <InlineCode>maxResolution</InlineCode> instead of
      <InlineCode>minZoomLevel</InlineCode> and <InlineCode>maxZoomLevel</InlineCode>. The zoom
      level makes the most sense for RPC and it's the simplest to use. However if zoom levels are
      added to or removed from the embedded map for whatever reason by the admin of the Oskari-based
      service the zoom level numbers may change. This shouldn't be a common thing to happen but it's
      worth knowing about. The scale/resolution limits are not affected if this happens, but they
      are not that easy to determine on an RPC-app. You have a reference for scale/zoom level on the
      <InlineCode>AfterMapMoveEvent</InlineCode> while resolutions are only used internally by the
      map implementation.
    </p>
  </div>
</template>
  ...
  methods: {
    ...
    addFeaturesToMap() {
      ...
      this.$root.channel.log(
        'MapModulePlugin.AddFeaturesToMapRequest posted with data',
        polygonParams
      );
    }
  }
